import constants
import pandas as pd
from operations import count, aggregate, average, merge


def extract_us_airport(df):
    """
    This function takes an airport dataFrame with 18 columns, only keeps "US" airport data by the "country" column,
    and returns a new dataFrame with 7 useful columns:
...
def get_flight_data_by_year(year, used_cols=[]):
    ...
    df_year = pd.read_csv(constants.ROOT + str(year) + '.csv')
    if not used_cols:
        return df_year
